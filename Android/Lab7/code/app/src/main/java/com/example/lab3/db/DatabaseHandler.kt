...
class DatabaseHandler(context: Context) :
    ...
    fun deleteUser(login: String) {
        val db = writableDatabase
        db.delete(DBContract.UserEntry.TABLE_NAME, "${DBContract.UserEntry.COLUMN_LOGIN}=?", arrayOf(login))
    }
    ...
    fun updateUserAvatar(login: String, avatarUri: String?) {
        val db = writableDatabase
        val values = ContentValues().apply {
            put(DBContract.UserEntry.COLUMN_AVATAR_URI, avatarUri)
        }
        db.update(
            DBContract.UserEntry.TABLE_NAME,
            values,
